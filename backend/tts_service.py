...
class TTSService:
    ...
    async def synthesize_speech(
        ...
        Args:
            ...
            audio_encoding: Audio encoding format (default: MP3)
        ...
        try:
            # Use Gemini-TTS API format with separate prompt and text fields
            # According to documentation: https://docs.cloud.google.com/text-to-speech/docs/gemini-tts
            synthesis_input = texttospeech.SynthesisInput(
                prompt=style_prompt,  # Style instruction
                text=text.strip()     # Actual text to speak
            )
            ...
            logger.info(f"Successfully synthesized speech using Gemini-TTS (model: {self.model}, voice: {voice_name}) for emotion: {emotion}, text length: {len(text)}")
